...
exports.cf43 = onRequest({
  ...
  try {
    ...
    logger.info(`[cf43] Starting process for ${isMaterial ? 'material' : 'product'}: ${targetRef.id}`);
...
//
    ...
    while (loopContinuously) {
      ...
      if (isLastAttempt && !wasEstimated) {
        logger.info(`[cf43] Max retries (${MAX_DIRECT_RETRIES}) reached. Triggering separate Estimation AI.`);
        wasEstimated = true;
...
//
          ...
          logger.info(`[cf43] Estimation complete.Probability: ${ supplier_probability_percentage }% `);
          logger.info(`[cf43] Estimation complete.Probability: ${ supplier_probability_percentage }% `);
          // Removed break; to allow flow to Fact Checker

        } catch (estErr) {
          logger.error(`[cf43] Estimation AI failed: ${ estErr.message } `);
...
//
      ...
      if (isSupplierUnknown(finalAnswer)) {
        logger.warn(`[cf43] Supplier is "Unknown" on attempt ${ retryCount + 1 }.`);

        // If we are already estimating and it's still Unknown, we must give up to avoid infinite loop
        if (wasEstimated) {
          logger.warn(`[cf43] Estimation returned Unknown.Giving up.`);
          break;
        }

        if (retryCount < MAX_DIRECT_RETRIES) {
          retryCount++;
          logger.info(`[cf43] Retrying(Retry #${ retryCount } / ${ MAX_DIRECT_RETRIES })...`);
          ...
          logger.info(`[cf43] Retries exhausted for Unknown, triggers estimation next loop.`);
          continue;
        }
      }
      ...
      if (collectedUrls.size === 0 && !wasEstimated) {
        logger.warn(`[cf43] Supplier found, but NO URLs collected.Cannot run Fact Checker.`);

        if (retryCount < MAX_DIRECT_RETRIES) {
          retryCount++;
          logger.info(`[cf43] Retrying due to missing URLs(Retry #${ retryCount } / ${ MAX_DIRECT_RETRIES })...`);

          currentPrompt = `...`;
          continue; // Loop again
        } else {
          logger.info(`[cf43] Retries exhausted for missing URLs.Triggering estimation next loop.`);
          continue;
        }
      }

      logger.info(`[cf43] Supplier found.Running Fact Checker...`);
...
let verifyModel = 'aiModel';
let verifyThinkingConfig = { includeThoughts: true, thinkingLevel: "HIGH" };

// 3b. Run Verification Loop (internal retry for format)
while (verifyAttempts < MAX_VERIFY_ATTEMPTS && !verifyResult) {
  verifyAttempts++;
  logger.info(`[cf43] Calling Verification (${verifyModel}, attempt ${verifyAttempts})...`);

  try {
    const collectedUrlsVerify = new Set();
    const rawVerifyResult = await runGeminiStream({
      model: verifyModel, // use variable
      generationConfig: {
//
//
//
...
//
    ...
    if (hasRating && hasReasoning) {
      ...
      if (collectedUrlsVerify.size > 0) {
        logger.info(`[cf43] Captured ${collectedUrlsVerify.size} URLs from Fact Checker.`);
        collectedUrlsVerify.forEach(url => collectedUrls.add(url));
      }

      logger.info('[cf43] Verification response format valid.');
    } else {
      logger.warn(`[cf43] Verification failed format check (attempt ${verifyAttempts}).`);
    ...
    logger.error(`[cf43] Verification attempt ${verifyAttempts} failed:`, verifyErr.message);

    // FALLBACK LOGIC: If 3.0 fails, try 2.5
    if (verifyModel === 'aiModel' && verifyAttempts < MAX_VERIFY_ATTEMPTS) {
      logger.info('[cf43] Switching to aiModel for fallback verification.');
      verifyModel = 'aiModel';
      verifyThinkingConfig = { includeThoughts: true, thinkingBudget: 24576 };
      // Do NOT break, allow loop to continue
      continue;
    }
    break; // Break if already 2.5 or max retries
  }
}
...
if (verifyResult) {
  ...
  logger.info(`[cf43] Fact Check Ratings parsed: ${ratings.length}`);
  ...
  if (isFactCheckFailed) {
    logger.warn(`[cf43] Fact check FAILED for ${badRatings.length} suppliers.`);

    if (wasEstimated) {
      logger.info("[cf43] Estimation fact check failed (low confidence). Saving as is with actual ratings.");
    ...
    if (retryCount < MAX_DIRECT_RETRIES) {
      retryCount++;
      logger.info(`[cf43] Retrying with feedback (Retry #${retryCount}/${MAX_DIRECT_RETRIES})...`);
      ...
      logger.info(`[cf43] Retries exhausted for Fact Check failure.`);
      ...
      if (goodRatings.length === 0) {
        logger.info(`[cf43] All suppliers failed verification. Triggering estimation next loop.`);
        ...
        if (!hasBetterThanProbable && !wasEstimated) {
          logger.info(`[cf43] Only found 'Probable' suppliers. Triggering estimation to attempt better results.`);
          continue;
        }

        logger.info(`[cf43] Some suppliers passed verification. Filtering and promoting...`);
        ...
        if (validSuppliers.length > 0) {
          ...
          logger.info(`[cf43] Saved filtered/promoted data: ${JSON.stringify(upd)}`);
    ...
    if (!hasBetterThanProbable && !wasEstimated) {
      logger.info(`[cf43] Fact Check passed but only found 'Probable' suppliers. Triggering estimation to attempt better results.`);
      continue;
    }

    logger.info(`[cf43] Fact check PASSED. Reshuffling and saving result.`);
    ...
    if (validSuppliers.length > 0) {
      ...
      await targetRef.update(upd);
      logger.info(`[cf43] Saved reshuffled data: ${JSON.stringify(upd)}`);
      ...
      logger.warn("[cf43] Fact Check passed but parsing failed for reshuffle. Saving as is.");
      finalRatings = ratings;
      break;
    }
  }
  ...
  logger.error(`[cf43] Fact checker crashed or failed format. Accepting main AI result to prevent stall.`);
  break;
}
    }
...
if (wasEstimated && finalAnswer !== "MANUALLY_HANDLED") { // Only process estimation parsing if not already handled by filter logic
  ...
  logger.info("[cf43] Processing estimated supplier response.");
  ...
  if (mainSuppMatch && mainSuppMatch[1]) {
    ...
    if (bestCandidate.rating <= 2 && bestCandidate.name.toLowerCase() !== 'unknown') {
      logger.info(`[cf43] Strong evidence (${bestCandidate.rating}) found for '${bestCandidate.name}'. Promoting to CONFIRMED.`);
      ...
      logger.info(`[cf43] Main supplier was Unknown. Promoted '${mainSupplier}' (Highest Probability) from other inputs.`);
  ...
  logger.info("[cf43] Processing direct supplier response.");
  ...
  if (otherSuppliers.length > 0) {
    ...
    logger.info(`[cf43] Found ${otherSuppliers.length} other known suppliers.`);
  }
  // --- END: New Logic ---

} else {
  logger.warn("[cf43] Loop finished without a valid supplier.");
}
...
if (Object.keys(upd).length > 0) {
  await targetRef.update(upd);
  logger.info(`[cf43] Saved parsed data: ${JSON.stringify(upd)}`);
}
...
await incrementChildProgress(materialId, 'cf43');
